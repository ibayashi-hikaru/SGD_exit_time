...
def get_model(config):
    if config["data"] == 'AVILA2':
        if config["model"]=='MLP':   model = AVILA2_model_zoo.MLP(config["sharpness"])
    elif config["data"]== 'MNIST':
        if config["model"]=='LeNet': model = MNIST_model_zoo.LeNet()
        elif config["model"]=='MLP':   model = MNIST_model_zoo.MLP()
    elif config["data"]== 'CIFAR10':
